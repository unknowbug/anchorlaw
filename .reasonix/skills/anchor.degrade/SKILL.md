...
> Protocol: spec/protocol-v0.7.md §9 (Degraded Verification)
> Layer: L2 (Anchors) — 与 anchor.test 同域（CLI 入口同为 anchorlaw test）
> Execution: subprocess

## 触发场景

RE-lifted 代码依赖二进制内部符号，无法独立编译运行；`anchorlaw test` 无法执行时。
...
4. **retry cap**（§9.4）：Lift→Verify 循环每函数**硬上限 3 次**——超过必须回到 A 层数据采集（动态 trace），禁止无新数据反复改假设（过程熵增）。
