...
| Judge-Driven Pipeline (v0.10; v0.11 amended; v0.13 scoped) | — | — | — | **Conjecture** | §15.1 four-stage pipeline defined (input contract → implementation spec → plan → parallel Workers → delivery); requirements discovery moved to a separate requirements protocol (Scout-driven, human-dialogue), Anchorlaw consumes its output; §15.4 acceptance-criteria-first (derived from input) + Judge-nod termination with mechanical fallback; v0.13: constructiveness claim scoped to the input-contract domain (registered in §11 after a §12 challenge), RE handover criterion added (§16.1), §9.4 retry cap upgraded to evidence saturation. Protocol-level design; no host has exercised the pipeline yet. Conformance tests ship with v0.13. |
...
- Current version: **v0.13** — pre-stable. All components subject to change based on practice feedback.
...
**Judge-driven pipeline (v0.10; v0.11 amended; v0.13 scoped)** — within this
...
pipeline ([§16.1](#161-definition) RE handover criterion). The workflow is
...
**RE handover criterion (v0.13)** — when the Host is a reverse-engineering
framework, the input contract counts as *confirmed* only when the
vanilla-behavior specification has **converged**: the RE host's discovery
loop has produced a stable behavioral model with no open hypotheses about the
target semantics that the implementation depends on. Mixed exploratory-constructive
tasks (e.g. a port whose acceptance criteria emerge while reverse-engineering
the target) MUST NOT enter the Judge-driven pipeline until the exploratory
portion has converged; the RE host either (a) completes the exploration first
and hands over a confirmed contract, or (b) calls Anchorlaw only for the
determined sub-portions whose behavioral model is already stable. Handing an
unconverged contract into the pipeline forces it to behave like an
exploration loop — out of domain ([§15.1](#151-definition)) — and collides
with the acceptance-criteria-first rule ([§15.4](#154-consistency-contract)).
