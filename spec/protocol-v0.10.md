...
| Judge-Driven Pipeline (v0.10) | — | — | — | **Conjecture** | §15.1 six-stage pipeline defined (requirements discovery ↔ Scout, human authorization gate, implementation spec, plan, parallel Workers, delivery); §15.4 acceptance-criteria-first + Judge-nod termination (replaces the v0.9 3-round cap); Scout/Worker reinstated as general-protocol roles. Protocol-level design; no host has exercised the pipeline yet. Conformance tests ship with v0.10. |
...
- Current version: **v0.10** — pre-stable. All components subject to change based on practice feedback.
...
| "Programming is Judge-driven: a six-stage pipeline terminates per stage on the Judge's nod" | §15.1 (v0.10) | Normative clause (pipeline model); no host has exercised it yet | ⚠️ scoped (unverified) |
| "Implementation MUST NOT begin before the human confirms the requirements at the human authorization gate" | §15.1/§16.1 (v0.10) | Normative clause (stage 2); host human confirm hook | ⚠️ scoped (unverified) |
| "The Judge determines acceptance criteria during requirements discovery, before implementation; criteria are part of the requirements document submitted to the human" | §15.4 (v0.10) | Normative clause (acceptance criteria first) | ⚠️ scoped (unverified) |
...
| "Module integration (stage 5) and delivery (stage 6) MUST be preceded by an isolated judge subprocess acceptance" | §15.4 (v0.10) | Normative clause (key-checkpoint isolated acceptance) | ⚠️ scoped (unverified) |
| "Scout (requirements analysis) and Worker (module implementation) are subprocess roles; their working context MUST NOT enter the main session" | §15.1/§15.2 (v0.10) | Normative clause (role reinstatement); host isolation capability | ⚠️ scoped (host capability) |
...
| **Judge** (role of the main session) | Holds the acceptance criteria; drives the pipeline end-to-end (requirements → human gate → spec → plan → integration → delivery); dispatches Scouts/Workers; adjudicates their outputs; grants or denies integration | Main-session context; at key checkpoints (module integration, delivery) MUST delegate independent verification to an isolated judge subprocess — self-review is not review ([§15.4](#154-consistency-contract)) |
| **Scout** (subprocess) | Reconnaissance: analyzes the request for clarity and feasibility; drafts implementation specs (naming conventions, modularization, framework boundaries) | Its full working context MUST NOT enter the main session; only the final answer and artifact references return |
...
**Judge-driven pipeline (v0.10)** — programming is not reverse engineering:
reverse engineering is exploratory (survey → discovery → convergence, unbounded
verification is its nature), while programming is constructive — it converges
on pre-defined acceptance criteria. The workflow is therefore **Judge-driven**,
a six-stage pipeline in which every stage terminates on a "Judge nod", not on
round counting:

1. **Requirements discovery** — the Judge dispatches Scouts to analyze the
   request; each analysis returns to the Judge, who judges clarity and
   feasibility. Unclear or infeasible parts are sent back for further Scout
   analysis. The stage ends when the Judge approves the **complete requirements
   document**.
2. **Human authorization gate** — the Judge MUST submit the complete
   requirements document together with the acceptance criteria (claims +
   validation carriers + boundary declarations, [§15.4](#154-consistency-contract))
   to the human. Implementation MUST NOT begin until the human confirms. This
   is the [§16.1](#161-definition) confirm hook applied at the
   requirements→implementation boundary; a Judge nod alone is a recommendation,
   not an authorization.
3. **Implementation spec** — the Judge dispatches Scouts (or Workers) to draft
   the implementation spec: naming conventions, modularization, framework
   boundaries. The Judge reviews the spec; only an approved spec enters
   planning.
4. **Implementation plan** — the Judge derives the module breakdown and the
   execution plan from the approved spec.
5. **Parallel implementation** — multiple Workers implement modules in
...
6. **Delivery** — when all modules are integrated, the Judge performs the
...
- A Scout MUST persist its requirements analysis / spec draft as an artifact
...
| Scout (subprocess) | `anchor.scout` | requirements analysis / spec drafting |
| Worker (subprocess) | `anchor.worker`, `anchor.write`, `anchor.test` | module implementation, anchoring, self-testing |

**Execution-mode selection principle (v0.10)** — convergent lightweight
actions (semantics lookup, single-file anchoring, verification runs, protocol
maintenance) default to `inline` in the main session. Divergent or heavy work —
surveys, large scans, requirements analysis, module implementation — runs as
subprocesses: Scouts and Workers execute isolated per
...
- (v0.10) MUST fire at every pipeline stage boundary — the Judge's nod is the
  stage's terminal action:
  1. **Requirements approval** — Judge approves the complete requirements
     document ([§15.1](#151-definition) stage 1 end);
  2. **Human-gate submission** — Judge submits requirements + acceptance
     criteria to the human and awaits confirmation (stage 2);
  3. **Spec approval** — Judge approves the implementation spec (stage 3 end);
  4. **Plan approval** — Judge approves the module breakdown (stage 4 end);
  5. **Module integration** — Judge reviews each Worker's module and grants or
     denies integration (stage 5, per module);
  6. **Delivery acceptance** — Judge performs the final review and, with the
     isolated acceptance verdict, authorizes delivery (stage 6).

**Acceptance criteria first (v0.10)** — the Judge determines the acceptance
criteria during requirements discovery, before implementation begins: the
claims the deliverable MUST satisfy, the validation carriers that bind them
(`@anchor.test` per [§5](#5-anchor-test-and-idk-decorators)), and the boundary
declarations (`@anchor.idk`) for what is out of scope. The criteria are part
of the complete requirements document submitted to the human at the human
authorization gate ([§15.1](#151-definition) stage 2). Implementation
converges on these criteria; a review opinion is judged against them, never
invented during review. Criteria-first is what makes the termination gates
below self-bounded — the criteria are the boundary, so verification cannot
run out of control.
...
**Key-checkpoint isolated acceptance (v0.10)** — the Judge (main session)
adjudicates continuously, but at two checkpoints independent acceptance is
MANDATORY: (1) before a Worker's module is integrated (stage 5), and (2)
before delivery (stage 6), the Judge MUST delegate a review to an isolated
...
(v0.10) The confirm hook is exercised at **two** points: the **human
authorization gate** ([§15.1](#151-definition) stage 2) — the human confirms
the complete requirements document + acceptance criteria before implementation
begins; and the **`confirmed` grant** — the human approves a delivery as
trustworthy ([§15.4](#154-consistency-contract)). Both are human-only: the
Judge's nod at any pipeline stage is a recommendation, never an authorization.
