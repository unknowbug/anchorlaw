...
- Current version: **v0.14** — pre-stable. All components subject to change based on practice feedback.
...
| "Pipeline iteration on the same acceptance criterion is capped at 3; iteration 4 MUST mechanically escalate to the human (criterion wrong → §12/amendment; approach wrong → planning)" | §15.4 (v0.12) | Normative clause (termination gates, C mechanical fallback; restores the v0.9 round cap adapted to criteria-first) | ⚠️ scoped (unverified) |
...
- **B. Review opinion severity tiers** — review/judge opinions fall into
  ...
  3. **info** — notes and observations; recorded, may be ignored. An opinion
     ...
     round.
- **C. Judge-nod termination with mechanical fallback (v0.10; v0.12
  ...
  **Mechanical fallback (v0.12; the v0.9 round cap, restored and adapted)** —
  ...
  being met, the pipeline MUST mechanically escalate to the human — no
  further iteration is allowed — regardless of whether the Judge perceives
  progress: either the criterion itself is wrong (challenge via
  [§12](#12-rule-challenge-process-v04) or human amendment) or the approach
  is wrong (return to planning). Rationale: the v0.10 Judge-nod-only form
  relied on the Judge recognizing "persistent failure" — the exact failure
  mode (repeated rounds without realizing) the gates exist to prevent.
  Criteria-first removes the cause of *criteria appearing after the fact*,
  but not the case of a *wrongly-defined criterion* that can never be met;
  the mechanical count is the fallback for that case. The per-function retry
