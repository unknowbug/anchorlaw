...
>
> v0.22: verification temporality, criterion preconditions, equivalence
> tiers, process invariants (2026-09-15) — five clauses closing gaps that
> practice had already paid for (CoreSwap #156/#160/#161/#162 + M11/M16),
> derived from an external study of the spatiotemporal-composability paper
> (`.investigations/paper-2608-25512-anchorlaw-proposals/`):
> ① **§9.8 Verification Temporality** — a verification action's file-system
> side effects MUST be registered with an addressable inverse (in-place) or
> declared irreversible with a reason; registration only, never automatic
> rollback (failed-round evidence is the more valuable artifact).
> ② **§15.1/§15.4 criterion preconditions** — an acceptance criterion
> carries the external facts it depends on; when one lapses the criterion is
> suspended and its dependents are marked premise-expired, and status is
> NEVER auto-changed. ③ **§9.7 equivalence tiers** — declares *under what
> conditions two executions may be judged observationally equivalent*,
> including the partial-equivalence honesty clause and an invalid-declaration
> list; the axis is named separately from the §9.1 capability modes to avoid
> key collision. ④ **§15.4 PI-1** — a halted pipeline is terminal and MUST
> NOT be continued as if its state were valid (PI-2 remains unverified and
> is labeled as such). ⑤ **§14 reference-integrity check** — the protocol
> applies §6.6's interface-drift analysis to its own citations, at the
> decidable layer only (version + clause-number existence + anchor
> resolution; semantic compatibility is undecidable and explicitly out of
> scope). Evidence status per clause is registered honestly in
> [§8](#8-maturity) / [§11](#11-universal-claim-audit-v04) — most are
> `scoped (no practice data yet)` by design: the clause exists so that hosts
> have one authoritative target to implement and to falsify.
>
> v0.21: DSH host adaptation capability parity + fail-closed preset gate
> (2026-09-15) — two host-adaptation gaps found by an upstream-drift audit
> (2026-09-09): ① the anchorlaw agent preset had silently fallen behind the
> upstream standard preset's capability surface; five rows are now tracked
> (`command-goal`, `tool-subagent-codex`, `tool-subagent-claude-code`,
> `tool-ralph`, `present`), with the three optional external-agent/workflow
> rows kept `disabled: true` exactly as upstream ships them (enabling
> requires installing the matching Bundle — host availability alone grants
> no tool). ② a `name:` in the composition that no longer resolves makes the
> whole preset fail to mount, and that surfaced only when a user tried to
> resume a session; preset-row resolvability is now a fail-closed selfcheck
> item (`dsh/tests/audit_preset_rows.mjs`, item 6), so an upstream rename or
> removal is caught at maintenance time instead. The protocol core is
> UNCHANGED — this is host-adaptation scope (§16), and it also closes the
> outstanding rename (`dsh-workflow-worker-thread` → `dsh-workflow-ptc`).
>
> v0.20: evidence/conclusion continuity (CoreSwap M11/M14/M16 proposal,
> 2026-09-01) — three gaps exposed by cross-session practice: ① conclusion
> supersession chain (§15.4 extension): a candidate+ conclusion overturned by
> later evidence MUST be expressed as a supersession record
> (`superseded_by`/`supersedes` + one-line reason); the original text is never
> deleted or rewritten, and the chain MUST mechanically answer "what is the
> currently valid conclusion about X, and its history". ② verification
> comparability statement (§9.7): any quantitative alignment/consistency
> metric MUST declare the comparison basis (carrier, coverage, comparability
> with prior metrics). ③ host handover validation (§16.3 checklist):
> handover items that are mechanism-directions/unverified hypotheses MUST be
> labeled with verification status, and the inheritor MUST run one cheap
> independent verification before using them as premises. Evidence
> persistence and compaction process remain host/framework scope (§16 hosts
> MAY) — the protocol defines semantics only.
>
> v0.19: verification-scope clarification (CoreSwap record-effectiveness
> assessment, 2026-08-15) — Anchorlaw is a VERIFICATION protocol, not a
> knowledge-accumulation protocol. The noise-card `discovery` / `curriculum`
> fields are reframed from "knowledge accumulation duty" to verification
> backtracking (evidence for why a claim/assertion mismatched), §15.2
> artifacts are stated as the verification-reproducibility carrier rather than
> "cross-session memory", and §14 is explicitly declared NOT the knowledge
> sink (that is the host / a separate knowledge mechanism's job). The
> verification core — `@anchor.test` / `source` / staleness / health states /
> §9 degraded — is UNCHANGED (it was the actual guard that let the seed
> contamination be traced, not misjudged as "Rust has no bug").
>
> v0.18: DSH host adaptation — DeepSeek Harness (DSH) is the first host
> implementing the full §16 Host Integration Contract interface surface
> (all four interface points): the `dsh/` subtree ships 11 anchor-* skills
> (§14 manifest, DSH format kebab-case + whenToUse), 4 model tools
> (anchorlaw_scan / anchorlaw_report / anchorlaw_ai_context /
> anchorlaw_status) with a host-level global tool mount through the profile
> patch layer, the anchorlaw agent preset (Judge-driven pipeline persona,
> §15.4), and a project-level (Reasonix-style) install mode for per-project
> skill scoping. §8 maturity and §11 audit rows updated from "no host has
> implemented the contract yet" to DSH evidence; DSH-side consistency is
> guarded by scripts/selfcheck.ps1 (6 items) and the manifest/schema tests.
>
> v0.17: §12 challenge outcomes (Reasonix/Go audit) — ① parse-error
> marker: an unparseable source file is a tool-level `parse-error` (INFO),
> never a P1-P6 defensive pattern (SyntaxError was misclassified as
> swallowed-exception, producing false ERROR counts); ② comment-form
> language claims downgraded: Go/Java registration means annotation-
> extraction ONLY — P1-P6 defensive-pattern detection is NOT mapped to
> comment-form languages (no false "registered = fully supported" claim);
> ③ four new language-agnostic reliability-risk patterns P7-P10 defined
> (LIFECYCLE / STATE_MACHINE / PATH-COORDINATION / COMPLEXITY), driven by
> the Go audit's real defect classes; implementations map them per
> language, Level 1 does not require them.
>
> v0.16: Go/Java language registration + Rust not-supported decision —
> Go and Java are registered as comment-form languages (line comments
> `// @anchor.*`, same declaration site as C++; independent probe/test
> binary as validation carrier) and wired into the reference extractor
> (annotation-extraction, same regex as C++). Rust is declared not
> supported by design: its compiler, borrow checker, and test framework
> already provide the verification this protocol adds elsewhere — an
> anchor layer would duplicate native guarantees; the earlier proc-macros
> plan is abandoned.
>
> v0.15: C-gate halted escalation — after 3 iterations on the same
> acceptance criterion without it being met (the review is still reporting
> unresolved issues), the pipeline MUST halt entirely: the Judge submits a
> detailed report of the review situation and unresolved issues to the
> human, who decides (criterion wrong → §12/amendment, approach wrong →
> planning, or otherwise). The Judge's own round-4 classification is
> removed — it is itself a convergence judgment that can keep failing. No
> further iteration, fix, or re-review without the human's decision.
>
> v0.14: input-contract layering + de-RE generalization — the input
> contract is clarified as a confirmed requirements document plus
> technical-constraint specification (customer-confirmable facts: claims,
> boundaries, terminology, technical constraints); architecture design
> (modularization, dependency direction, interfaces) is produced at pipeline
> stage 1, not carried in the input — closing the stage-0/stage-1 overlap.
> §16.1's RE handover criterion is generalized to a protocol-neutral
> input-contract confirmation criterion (semantic convergence), removing
> RE-specific adaptation: the three protocols (requirements, Anchorlaw,
> reverse-engineering) are independently operable frameworks — Anchorlaw
> consumes Scout's output, a discovery framework references Anchorlaw, and
> Anchorlaw adapts to no upstream beyond the interfaces it must provide.
>
> v0.13: §12 challenge outcome (RE report: "programming is constructive"
> was an unregistered universal claim) — the claim is now SCOPED to the
> input-contract domain and registered in the §11 audit; reverse
> engineering is explicitly OUTSIDE the domain (exploratory, unbounded
> verification), the two modes are complementary, and §16.1 defines the RE
> handover criterion (a contract counts as confirmed only when the
> vanilla-behavior model has converged; mixed tasks enter Anchorlaw only
> for determined sub-portions). §9.4 retry cap upgraded from "3 rounds" to
> "3 rounds WITHOUT new data-layer evidence" (evidence saturation) — an
> exploration acquiring new evidence each round is never cut off; the gate
> stays mechanical because the presence of new evidence is mechanically
> decidable. AGENTS.md residual assertion cleaned (index-only discipline).
>
> v0.12: C-gate mechanical fallback restored — criteria-first remains the
> core (acceptance criteria determined before implementation, Judge-nod
> termination per stage), and the v0.9 3-round cap is restored in adapted
> form: iteration on the SAME acceptance criterion is tracked in the plan
> and capped at 3; iteration 4 MUST mechanically escalate to the human
> (criterion wrong → §12/amendment; approach wrong → planning). The v0.10
> Judge-nod-only form relied on the Judge recognizing persistent failure —
> the exact failure mode (repeated rounds without realizing) the gates
> exist to prevent; the mechanical count is the fallback for wrongly-defined
> criteria. Acceptance criteria must also be persisted as an artifact
> (v0.11 amend) for delivery-acceptance traceability.
>
...
| Change | Trigger | Section |
|--------|---------|---------|
| **Verification Temporality** — a verification action's file-system side effects MUST be registered: `in-place` effects (overwriting a log, rebuilding an index, changing a gate default) MUST carry an addressable inverse or an explicit irreversible declaration with a reason; `derived` effects (a newly named artifact, a fresh temp dir) are compliant by construction. Registration only — automatic rollback is explicitly forbidden (it would destroy failed-round evidence). | CoreSwap: three real evidence losses on record — 260913-05 A2 first-round log overwritten by a same-label rerun (judge MUST-1 could only close with an honesty statement), 260917-03 K2-D5 half log, 260913-06 failed first run; `#144`/`#146` exist as project discipline but live in handover docs, not in the protocol; `merge_index.py` rebuild-style writes silently drop non-whitelisted fields (live data-loss risk). External study: the source paper explicitly places the inverse's correctness on the component author, NOT on the runtime — so a declaration guarantee is the accurate claim, not a structural one. | [§9.8](#98-verification-temporality-v022) |
| **Criterion preconditions** — an acceptance criterion carries the external facts it depends on (`key` / `expected` / `check`); when a precondition lapses the criterion is **suspended** and its dependents are marked premise-expired. Status is NEVER auto-changed (§15.4 one-way transitions, §16.1 confirm hook). | CoreSwap #161: a criterion's observation object (world identity) had silently lapsed — the mechanical read still produced a "normal-looking" number, and the judge three-source cross-check did not catch it; #156 (whole arm VOID: a master switch was lost in the rerun basis), M11 seed mismatch (three occurrences). The §15.4 C gate counts *rounds* and cannot detect "the premise quietly expired". | [§15.1](#151-definition) / [§15.4](#154-consistency-contract) |
| **Equivalence tiers** — declares under what conditions two executions may be judged observationally equivalent: `E1 same-carrier single-variable` / `E2 cross-carrier or cross-form` (compare only the mutually declared observation-key set `S`) / `E3 interleaving-unknown` (equivalence is undecidable; only "no difference observed under the recorded interleaving"). Includes the partial-equivalence honesty clause (prove no branching outside `S` before declaring equivalence) and an invalid-declaration list. Named on its own axis to avoid collision with the §9.1 capability modes. | CoreSwap #162: the main session's "46/46 = ABI switch" comparability *explanation* looked reasonable and was refuted by a 10-line log sample — §9.7 requires declaring a basis but nothing intercepts a wrong declaration; #115 (content-fingerprint gate with an explicit "does not cover write-back/concurrency" boundary) is the practice this clause generalizes; concurrency/interleaving was never covered by §9.7 (MT8/9/10, #107, #113). External study: `≃_S` is only a *partial* equivalence on functions and iterators — reflexivity is not preserved, so "looks the same" is not a licence to compare. | [§9.7](#97-verification-comparability-statement-v020) |
| **Process invariant PI-1 (halt is terminal)** — a halted pipeline MUST NOT be continued as if its state were valid. PI-2 (unique adjudication endpoint for fan-out candidates) is registered as an unverified direction, pending a criterion-dependency acyclicity check. | CoreSwap #160: the gate only printed instead of exiting, so a VOID boot's contaminated artifacts became the next boot's `prev` — the halt never became a quiescent state; §15.4 says halt-and-human-decides but has no clause forbidding silent inheritance. External study: the paper's Progress/Confluence theorems *assume* acyclicity (stated as an assumption, not a theorem) — the honest move is to carry that premise over and register it, not to pretend it holds. | [§15.4](#154-consistency-contract) / [§11](#11-universal-claim-audit-v04) |
| **Reference-integrity check (decidable layer only)** — the protocol applies §6.6's interface-drift/key-collision analysis to its own citations: live reference version numbers, cited clause-number existence (same number, changed meaning = the clause-level form of key collision), and anchor resolvability. Historical archives are excluded; semantic compatibility is undecidable and explicitly out of scope. | External study of the paper's §6.6 (interface drift: a consumer's declared `k` is satisfied at the coeffect layer while the runtime value no longer matches); CoreSwap #156 (a citation lost its semantic premise in transit), #13 (transcription distortion), #90 (handover-item drift). §15.4 has been amended repeatedly (v0.13→v0.20) while citing text stayed put. | [§14](#14-agent-skills-anchor-skill-manifest-v05) / [§11](#11-universal-claim-audit-v04) |
| **Version bump** v0.21 → v0.22 | Five clauses absorbing CoreSwap practice evidence (verification temporality / criterion preconditions / equivalence tiers / PI-1 / reference integrity) | [§10](#10-versioning) |

## Changelog from v0.20

| Change | Trigger | Section |
|--------|---------|---------|
| **DSH preset capability parity** — the anchorlaw preset now tracks the upstream standard preset's row surface: `command-goal`, `tool-subagent-codex`, `tool-subagent-claude-code`, `tool-ralph`, `present` added; the three optional external-agent/workflow rows stay `disabled: true` exactly as upstream ships them (enabling requires installing the matching Bundle in the Profile) | upstream-drift audit (2026-09-09 + 2026-09-15 re-check): the preset had silently fallen behind the standard preset's capability surface while a plugin rename was also outstanding | [§8](#8-maturity) / [§16](#16-host-integration-contract-v06) |
| **Fail-closed preset-row resolvability gate** — `dsh/tests/audit_preset_rows.mjs` checks that every composition `name:` resolves against the harness package set (package + `exports` subpath) and that the installed copy's local `./` files exist; wired as `selfcheck.ps1` item 6; any unresolvable row exits non-zero | 2026-09-09 incident: an upstream rename left a stale package reference in the source composition — preset mount failed and sessions could not be created/resumed; the failure surfaced only at user resume time, never at maintenance time | [§8](#8-maturity) / [§14](#14-agent-skills-anchor-skill-manifest-v05) |
| **Upstream plugin rename followed** — `@deepseek-ai/dsh-workflow-worker-thread` → `@deepseek-ai/dsh-workflow-ptc` (`id` + `name`; `config.provider: spawn` unchanged, matching the official assembly) | 2026-09-09 upstream drift report; verified against harness `0d1f50007f` (`dsh-v0.1.6-alpha.1`) | [§8](#8-maturity) / [§16](#16-host-integration-contract-v06) |
| **Version bump** v0.20 → v0.21 | DSH host adaptation capability parity + fail-closed preset-row gate | [§10](#10-versioning) |

## Changelog from v0.19

| Change | Trigger | Section |
|--------|---------|---------|
| **Conclusion supersession chain** — a candidate+ conclusion overturned by later evidence MUST be expressed as a supersession record (bidirectional `superseded_by`/`supersedes` links + one-line reason); the original text is never deleted or rewritten; the chain MUST mechanically answer "currently valid conclusion + history"; staleness is one trigger, carrier is host scope | CoreSwap M14/M16: a handover "mechanism direction (candidate, unverified)" was treated as an axiom by the next session (wasted round); its overturning had no supersession expression (only a hand-written voided marker) | [§15.4](#154-consistency-contract) |
| **Verification comparability statement** — any quantitative alignment/consistency metric MUST declare the comparison basis: carrier, coverage, comparability with prior metrics (non-comparable → one-line difference statement) | CoreSwap M16: probe-domain 96.06% vs archive-write 82.16% nearly compared directly, misread as a "14pp regression" — different carriers, no comparability | [§9.7](#97-verification-comparability-statement-v020) |
| **Host handover validation** — handover documents distinguish verified conclusions from mechanism-directions/unverified hypotheses (labeled with verification status); the inheritor MUST run one cheap independent verification (≤1 round) before using a direction-level conclusion as a premise | CoreSwap M11/M14 common root cause: an unverified hypothesis crossed a session boundary and was promoted to an axiom; the checklist covered "what was handed over" but not "which parts are axioms vs assumptions" | [§16.3](#163-integration-checklist) |
| **Version bump** v0.19 → v0.20 | Evidence/conclusion continuity (supersession + comparability + handover validation) | [§10](#10-versioning) |

## Changelog from v0.18

| Change | Trigger | Section |
|--------|---------|---------|
| **Verification-scope clarification** — Anchorlaw is a VERIFICATION protocol, not a knowledge-accumulation protocol: §3 noise-card `discovery`/`curriculum` reframed from "knowledge accumulation duty" to verification backtracking; §15.2 artifacts stated as the verification-reproducibility carrier (not "cross-session memory"); §14 explicitly declared NOT the knowledge sink | CoreSwap record-effectiveness assessment (2026-08-15): users' record burden came from Anchorlaw's fields overlapping knowledge accumulation (which is a host / separate knowledge mechanism's job), not from the verification core itself — the verification traceability (source/staleness/health states/§9) was what actually let the seed contamination be traced | [§3](#3-noise-card-json-schema) / [§15.2](#152-isolation-semantics) / [§14](#14-agent-skills-anchor-skill-manifest-v05) |
| **Version bump** v0.18 → v0.19 | Verification-scope clarification | [§10](#10-versioning) |

## Changelog from v0.17

| Change | Trigger | Section |
|--------|---------|---------|
| **DSH host adaptation** — DeepSeek Harness (DSH) host adaptation layer (`dsh/` subtree) ships 11 anchor-* skills (DSH format, kebab-case + whenToUse), 4 model tools (anchorlaw_scan/report/ai_context/status), the anchorlaw agent preset (Judge-driven pipeline persona), a host-level global tool mount (profile `cordis.patch.yml` insert) and a project-level (Reasonix-style) install mode | First full §16 Host Integration Contract implementation: DSH implements all four interface points (execution / artifact paths / confirm hook / trigger point); §11 audit rows updated from "no host has implemented the contract yet" | [§16](#16-host-integration-contract-v06) / [§8](#8-maturity) / [§11](#11-universal-claim-audit-v04) |
| **Version bump** v0.17 → v0.18 | DSH host adaptation registered | [§10](#10-versioning) |
| **noise resolve suffix matching fixed (post-release, 2026-08-15)** — `anchorlaw noise resolve` accepts the short display suffix printed by `noise list` (the last 8 chars of the full id, via `ai_context_entry`) and the hex part without the `noise-` prefix, honoring the CLI help contract "Noise ID (or suffix)"; a suffix matching several cards is refused as ambiguous; 4 new unit tests (suite 102 green) | DSH maintenance session live check: `NoiseStore.resolve` did exact-match lookups only, so the id shown by `noise list` could never be used to resolve — reproduced (`resolve <suffix>` → FAIL), fixed with `_find()` suffix matching, e2e re-verified (`resolve <suffix>` → PASS) | [§3](#3-noise-card-json-schema) / noise CLI |

## Changelog from v0.16

| Change | Trigger | Section |
|--------|---------|---------|
| **parse-error marker** — an unparseable source file is a tool-level `parse-error` (INFO), never a P1-P6 pattern; reference scanner implements it | §12 challenge (Reasonix/Go audit): SyntaxError fallback was misclassified as swallowed-exception, producing false ERROR counts | [§6](#6-scanner-pattern-catalog) / reference scanner |
| **Comment-form claims downgraded** — Go/Java (and C++) registration means annotation-extraction ONLY; P1-P6 detection is not mapped to comment-form languages | Reasonix/Go audit: "registered = fully supported" impression vs. zero defect detection on a 2441-file Go project | [§2.4](#24-cross-language-notes) / [§13](#13-protocol-generality-anchor-abstraction-v04) / [§8](#8-maturity) / [§11](#11-universal-claim-audit-v04) |
| **P7-P10 reliability-risk patterns** — LIFECYCLE / STATE_MACHINE / PATH-COORDINATION / COMPLEXITY defined language-agnostically | Reasonix/Go audit: real defect classes (shutdown hang, session stuck, silent no-op, repeated full scans) are outside P1-P6; language-agnostic definitions, implementations map per language, Level 1 does not require them | [§6](#6-scanner-pattern-catalog) / [§7](#7-implementation-compliance-levels) |
| **Version bump** v0.16 → v0.17 | §12 challenge outcomes | [§10](#10-versioning) |

## Changelog from v0.15

| Change | Trigger | Section |
|--------|---------|---------|
| **Go/Java language registration** — Go and Java registered as comment-form languages (line comments, same declaration site as C++; independent probe/test binary as validation carrier); reference extractor wired (annotation-extraction) | cross-language support request: the anchor abstraction is language-agnostic (§13); Go/Java share the C++ line-comment form, so registration + extractor wiring is minimal | [§2.4](#24-cross-language-notes) / [§13](#13-protocol-generality-anchor-abstraction-v04) / [§8](#8-maturity) |
| **Rust not supported by design** — Rust's compiler, borrow checker, and test framework already provide the verification this protocol adds elsewhere; the proc-macros plan is abandoned | design decision: an anchor layer would duplicate native Rust guarantees | [§2.4](#24-cross-language-notes) / [§11](#11-universal-claim-audit-v04) |
| **Version bump** v0.15 → v0.16 | Go/Java registration + Rust decision | [§10](#10-versioning) |

## Changelog from v0.14

| Change | Trigger | Section |
|--------|---------|---------|
| **C-gate halted escalation** — after 3 iterations on the same acceptance criterion without it being met, the pipeline MUST halt entirely; the Judge submits a detailed report of the review situation and unresolved issues to the human, who decides — no Judge pre-classification at round 4 | Practice: a round-3 review still reporting unresolved issues must not continue as a Judge-classified escalation — the classification itself (criterion-wrong vs approach-wrong) is a convergence judgment that can keep failing | [§15.4](#154-consistency-contract) / [§11](#11-universal-claim-audit-v04) |
| **Version bump** v0.14 → v0.15 | C-gate escalation hardened | [§10](#10-versioning) |

## Changelog from v0.13

| Change | Trigger | Section |
|--------|---------|---------|
| **Input-contract layering** — input contract = confirmed requirements + technical-constraint specification (customer-confirmable facts); architecture design (modularization, dependency direction, interfaces) is produced at pipeline stage 1 | req-scout design review: "software specification" in the input contract overlapped stage-1 spec drafting; Scout cannot write design judgments (role rules) — the contract must carry facts, not design | [§15.1](#151-definition) / [§11](#11-universal-claim-audit-v04) |
| **§16.1 generalized — de-RE** — RE handover criterion becomes a protocol-neutral input-contract confirmation criterion (semantic convergence); no upstream framework named | Three-protocol independence: Anchorlaw must not adapt to any upstream; the interface it must provide is "what counts as a confirmed contract", stated without naming the RE framework | [§16.1](#161-definition) |
| **Version bump** v0.13 → v0.14 | input-contract boundary clarified | [§10](#10-versioning) |

## Changelog from v0.12

| Change | Trigger | Section |
|--------|---------|---------|
| **Domain-scoped constructiveness claim** — "programming is constructive" narrowed to the input-contract domain and registered in the §11 audit | §12 challenge (RE report: unregistered universal claim; CoreSwap SearchTree/biome tie-break as mixed-task counter-evidence) | [§15.1](#151-definition) / [§11](#11-universal-claim-audit-v04) |
| **RE handover criterion** — the input contract counts as *confirmed* only when the vanilla-behavior model has converged; mixed exploratory-constructive tasks MUST NOT enter the pipeline until the exploratory portion converges | RE report: RE programming collides with acceptance-criteria-first when the contract is unconverged | [§16.1](#161-definition) |
| **§9.4 evidence saturation** — retry cap counts 3 rounds WITHOUT new data-layer evidence, not 3 rounds; evidence-producing rounds reset the streak | RE report: "3-round cap cuts off an exploration that converges on round-5 trace evidence"; same anti-entropy motivation, precise criterion | [§9.4](#94-retry-cap-anti-entropy) |
| **AGENTS.md residual assertion cleaned** — "not reverse engineering" constructiveness assertion removed from the index | Mirroring discipline: index must not carry protocol assertions | [AGENTS.md](../AGENTS.md) |

## Changelog from v0.11

| Change | Trigger | Section |
|--------|---------|---------|
| **C-gate mechanical fallback restored (v0.12)** — iteration on the same acceptance criterion is capped at 3 in the plan; iteration 4 MUST mechanically escalate to the human; Judge-nod termination retained as the normal completion path | protocol-layer audit (v0.8→v0.11): the v0.10 Judge-nod-only form relied on the Judge recognizing persistent failure — the exact failure mode (repeated rounds without realizing) the gates exist to prevent; criteria-first removes criteria-appearing-after-the-fact but not wrongly-defined criteria; the mechanical count is the fallback | [§15.4](#154-consistency-contract) |
| **Acceptance-criteria artifact (v0.11 amend, carried into v0.12)** — derived acceptance criteria MUST be persisted under the artifact store (default `.artifacts/`) for delivery-acceptance traceability | post-release amend: acceptance was checkable only against memory; the criteria artifact makes the delivery gate auditable, consistent with §5.5 source artifact | [§15.1](#151-definition) |
| **Version bump** v0.11 → v0.12 | C-gate mechanical fallback restored | [§10](#10-versioning) |
...
| **Acceptance-criteria artifact (v0.11 amend)** — derived acceptance criteria MUST be persisted under the artifact store (default `.artifacts/`) for delivery-acceptance traceability | post-release amend: acceptance was checkable only against memory; the criteria artifact makes the delivery gate auditable, consistent with §5.5 source artifact | [§15.1](#151-definition) |
...
- **Rust**: not supported by design (v0.16) — Rust's compiler, borrow
  checker, and test framework already provide the verification this protocol
  adds elsewhere; an anchor layer would duplicate native guarantees. The
  earlier "proc macros" plan is abandoned ([§11](#11-universal-claim-audit-v04)).
- **Go (v0.16; v0.17 scoped)**: line comments `// @anchor.test("description", source="...")`
  — same declaration site as C++; comments are inert by construction; the
  validation carrier is an independent probe/test binary. Implemented in the
  reference extractor. **Scope (v0.17)**: registration means
  annotation-extraction ONLY — P1-P6 defensive-pattern detection is NOT
  mapped to comment-form languages (no defect detection until implemented).
- **Java (v0.16; v0.17 scoped)**: line comments `// @anchor.test("description", source="...")`
  — same declaration site as C++; comments are inert by construction; the
  validation carrier is an independent probe/JUnit binary. Implemented in the
  reference extractor. **Scope (v0.17)**: annotation-extraction ONLY — P1-P6
  not mapped (no defect detection until implemented).
- **All languages (scoped)**: For implemented languages (Python, TypeScript, C++, Go, Java), anchors MUST be removable by deleting a single dependency declaration, and annotations in source MUST be inert when the dependency is absent. Rust is not supported by design (native verification).
...
```json
{
  ...
  "properties": {
    ...
    "discovery": {
      "type": "string",
      "description": "Verification backtracking: which claim/assertion did this failure reveal as mismatched, and what evidence contradicts it (records WHY we now judge differently — it does not accumulate generalizable knowledge; that is a host / separate knowledge mechanism's job, not Anchorlaw's)."
    },
    "curriculum": {
      "type": "string",
      "description": "Optional, verification-backtracking lesson for AI context injection — an actionable note on how this failure should adjust a verification claim. NOT a knowledge-base entry; reusable knowledge accumulation is a host / separate mechanism responsibility."
    },
    "converted_to_test": {
      "type": "string",
...
**Parse errors are NOT patterns (v0.17)** — an unparseable source file is a
tool-level `parse-error` marker (INFO severity), never classified as any
P1-P6 defensive pattern. A §12 challenge (Reasonix/Go audit) found the
SyntaxError fallback misclassified as swallowed-exception, producing false
ERROR counts; the reference scanner now emits `parse-error` (INFO).

#### P7-P10: Reliability-risk patterns (v0.17, language-agnostic)

Driven by the Reasonix/Go practice audit — real defect classes that P1-P6
(defensive-coding signals) do not cover. Defined language-agnostically
(§6 catalog extension); implementations map them per language (§13) and
Level 1 does not require them (§7).

| Pattern | Severity | Definition (language-agnostic) | Signal |
|---------|----------|-------------------------------|--------|
| **P7 LIFECYCLE** | WARNING | A resource is acquired with a release path, but the release has no timeout and no fallback — shutdown/cleanup can hang or block indefinitely | resource-recovery hang (e.g. shutdown without timeout) |
| **P8 STATE_MACHINE** | WARNING | A state can be entered with no corresponding exit event and no timeout fallback — the machine can remain in the state forever | permanent-state rotation (e.g. session stuck in a loop) |
| **P9 PATH-COORDINATION** | WARNING | An entry point short-circuits on an empty path, so the downstream flow silently does not execute | silent no-op on empty input (e.g. reconcile skips a directory) |
| **P10 COMPLEXITY** | INFO | The same data is fully traversed multiple times (repeated O(n) full scans) — latency risk on large inputs | repeated full scans (e.g. long-session load) |
...
| Level | Requirements |
|-------|-------------|
| **Level 1 — Scanner** | Implements P1-P6 with severity layering (6.1) and registry awareness (6.2). CLI. P7-P10 (v0.17 reliability-risk patterns) are language-agnostic definitions — implementations map them per language and Level 1 does NOT require them. |
...
| Component | Python | TypeScript | C++ / Go / Java (comment-form) | Maturity | Latest Evidence |
|-----------|--------|-----------|-----------|----------|----------------|
| Scanner | ✅ | ✅ | annotation-extraction | **Verified** | photo_screener: 38 findings 0 FP; 55 unit tests (v0.4); severity layering verified by tests. **v0.17**: comment-form languages (C++/Go/Java) are annotation-extraction ONLY — P1-P6 detection is not mapped to them; P7-P10 reliability-risk patterns defined (implementation pending) |
...
| Host Integration Contract (v0.6) | — | — | — | **Scoped (DSH implements all four points; practice pending)** | §16 interface points defined. CoreSwap acted as first practice host (boundary clauses held; no framework restructuring). **v0.18: DSH (dsh/ subtree, 2026-08-13) is the first host implementing all four interface points** — trigger point: 11 anchor-* skills (§14 manifest, DSH format); execution point: 4 anchorlaw_* tools driving the anchorlaw CLI; confirm hook: anchorlaw preset Judge persona (`confirmed` granted only by the human, §15.4); artifact paths: `.artifacts/`/`.investigations/` conventions. Guarded by scripts/selfcheck.ps1 (6 items) + tests/test_manifest.py + tests/check_plugin_schema.mjs + tests/audit_preset_rows.mjs. **v0.21: preset capability parity with the upstream standard preset** (5 rows tracked; the three optional external-agent/workflow rows stay `disabled: true` as upstream ships them) **+ fail-closed preset-row resolvability gate** (selfcheck item 6). Pipeline trial data pending. |
...
| Judge-Driven Pipeline (v0.10; v0.11 amended; v0.13 scoped; v0.14 layered) | — | — | — | **Conjecture** | §15.1 four-stage pipeline defined (input contract → implementation spec → plan → parallel Workers → delivery); requirements discovery moved to a separate requirements protocol (Scout-driven, human-dialogue), Anchorlaw consumes its output; §15.4 acceptance-criteria-first (derived from input) + Judge-nod termination with mechanical fallback; v0.13: constructiveness claim scoped to the input-contract domain (registered in §11 after a §12 challenge), §9.4 retry cap upgraded to evidence saturation; v0.14: input contract layered (customer-confirmable facts only — architecture design produced at stage 1), §16.1 handover criterion generalized to a protocol-neutral input-contract confirmation criterion; v0.15: C-gate escalation hardened — 3 iterations on the same acceptance criterion unmet → the pipeline MUST halt entirely, the Judge submits a detailed report of the review situation and unresolved issues, and the human decides (no Judge pre-classification). Protocol-level design; no host has exercised the pipeline yet. Conformance tests ship with v0.15. |
| Verification Temporality (v0.22) | — | — | — | **Scoped (practice evidence exists; no compliant implementation yet)** | §9.8 defined. Trigger evidence is first-hand and recorded (CoreSwap three evidence losses: 260913-05 A2 overwritten first-round log, 260917-03 K2-D5 half log, 260913-06 lost failed first run; #144/#146 as project-only discipline). Two practice-side precedents already exist and are the shape the clause generalizes: unique temp zone for one-shot scripts, and "archive before same-label rerun". Not yet implemented: no host registers inverses per §9.8, and no scan gate checks §9.8 compliance. |
| Criterion Preconditions (v0.22) | — | — | — | **Scoped (trigger evidence exists; mechanism is a lift, not an invention)** | §15.1/§15.4 precondition set defined (`key`/`expected`/`check`), suspension semantics defined, status-immunity boundary stated. Trigger evidence: CoreSwap #161 (world-identity premise lapsed; judge three-source also missed it), #156 (master switch lost in rerun basis → whole arm VOID), M11 seed mismatch (three occurrences). Adoption path already exists in practice: the collection pipeline's self-certification gate is already a satisfaction check that voids a round — the clause lifts it into the criteria artifact. No host has implemented the artifact field yet. |
| Equivalence Tiers (v0.22) | — | — | — | **Scoped (practice precedents exist; tier adoption pending)** | §9.7.1 defined: E1/E2/E3 tier axis (named separately from §9.1 capability modes to avoid key collision), partial-equivalence honesty clause, four invalid-declaration forms. Practice precedents the clause generalizes: content-fingerprint gate with an explicit coverage boundary (#115), same-binary single-variable A/B (#150), per-file sha equality gate (#116), run-to-run noise anchors (#111/#148). Trigger evidence: #162 (a plausible comparability narrative refuted by a 10-line sample) and the interleaving family (MT8/9/10, #107, #113) that §9.7 never covered. |
| Process Invariants (v0.22) | — | — | — | **PI-1: Scoped (script-level implementation exists) / PI-2: Unverified (premise unregistered)** | PI-1 (halt is terminal, no silent inheritance) has a script-level precedent at the practice host (#160 fix) but no protocol-level conformance test. PI-2 (unique adjudication endpoint) is explicitly registered as **unverified**: the acyclicity premise is an assumption in the source analysis and has not been enumerated in practice; hosts MUST run the ≤1-round dependency-graph enumeration before adopting it, and amend to "cycles permitted but registered" if cycles are found. |
| Reference Integrity (v0.22) | — | — | — | **Scoped (decidable layer defined; checker not implemented)** | §14.7 defined for the decidable layer only (live-reference version, clause-number existence, anchor resolvability); semantic compatibility explicitly excluded as undecidable; historical archives explicitly excluded. Trigger evidence: #156/#13/#90 drift forms + repeated §15.4 amendment history. Carrier exists (the §11 audit table). Ownership assigned to protocol maintenance, not hosts. |
...
When Partial or Degraded mode is active, the implement→verify cycle is bounded by **evidence saturation**: after **3 consecutive hypothesis-verification rounds that produce no new data-layer evidence** (trace/probe), the methodology MUST force a return to data-layer collection (dynamic tracing) — escalating to the human rather than iterating blindly. A round that DOES produce new data-layer evidence resets the no-evidence streak and does not consume the cap: an exploration that is genuinely acquiring evidence each round is never cut off mid-evidence (the exact failure mode a §12 v0.13 challenge flagged).

Rationale: repeatedly tweaking hypotheses (implemented code) without new data-layer evidence (traces) is the definition of process entropy (实践偏离:过程熵增). The cap breaks this cycle by refusing to let the implementation iterate in isolation. "3 rounds" is the mechanical approximation; "3 rounds WITHOUT new evidence" is the precise criterion — the presence of new evidence is mechanically decidable (a new data point: a trace/probe record that does not duplicate a previously recorded one), so the gate stays mechanical while not penalizing evidence-producing iterations.

**Scope Clarification (v0.7; v0.13 amended):** the cap counts **hypothesis-verification rounds with no new data-layer evidence** — repeatedly adjusting a hypothesis (a model of *how the mechanism works*) with no new data-layer evidence (trace/probe) is process entropy and consumes the cap. **Engineering fixes do NOT consume the cap**: fixing implementation defects (compilation failures, crashes, runtime errors) may iterate indefinitely until correct. Distinguishing criterion:
- Hypothesis verification without new evidence: changes the *understanding* of the mechanism but produces no new data-layer evidence → consumes the no-evidence streak
- Hypothesis verification with new evidence: produces a new trace/probe record → resets the streak, does not consume the cap
...
### 9.7 Verification Comparability Statement (v0.20)

Beyond the Degraded Mode Honesty Statement ([§9.5](#95-degraded-mode-honesty-statement-v07)), any output of a quantitative alignment/consistency metric MUST declare the **comparison basis** in three elements:

1. **Carrier** — in which representation domain / artifact layer each side is compared (e.g. raw-id probe domain self-consistency vs cross-domain write-decoded bit-exactness).
2. **Coverage** — sample scope: single-point or batch, which generation stages are included, what is excluded.
3. **Comparability with prior metrics** — whether the metric continues a historical measurement line; if not comparable, a one-line statement of the difference is REQUIRED.

Rationale (CoreSwap M16): a probe-domain self-consistent 96.06% and an archive-write-level 82.16% were nearly compared directly and misread as a "14pp regression" — the two carriers differ (raw-id domain self-consistency vs cross-domain write-decoded per-bit) and have no comparability whatsoever. Metrics without a declared basis invite cross-caliber misjudgment; the declaration is the quantitative sibling of the §9.5 honesty statement.

#### 9.7.1 Equivalence Tiers (v0.22)

The three elements above state *what* must be declared. The tiers state *when two executions may be judged observationally equivalent at all*. The tier names are deliberately on their own axis (`E1`/`E2`/`E3`), NOT reusing the §9.1 capability modes (`Full`/`Partial`/`Degraded`): the two axes are independent, and sharing names across meanings is the clause-level form of key collision this protocol audits for.

| Tier | Form | What may be claimed |
|------|------|---------------------|
| **E1 — same carrier, single variable** | One execution body, zero recompilation, runtime flag override | Observational equivalence MAY be asserted constructively (both sides are the same binary, so the "no branching outside `S`" premise below is discharged by construction). This is the only tier that supports magnitude-level conclusions. |
| **E2 — cross carrier or cross form** | Different binary, different build, different gating form | Equivalence MUST be read as **restriction to the mutually declared observation-key set `S`** (e.g. the changed set, the fingerprinted chunk set, the per-block comparison denominator). Components outside `S` (a `common`-count gap, a section-count semantic) MUST NOT enter the equivalence judgement. E2 is a **normal** tier, not a failure mode — cross-version and cross-implementation work lives here permanently. |
| **E3 — interleaving unknown** | The two observations were not taken under the same interleaving (concurrency, parallel arms, probe interference) | Equivalence is **not decidable**. Only "no difference was observed under the recorded interleaving" may be claimed, worded in the shape of the §9.5 honesty statement. |

**Partial-equivalence honesty clause.** The underlying relation is only a **partial** equivalence: it is symmetric and transitive, but **reflexivity is not preserved**. Two executions that "look the same" may branch on a key outside `S`, and the relation then simply does not apply — respect is a *condition*, not a given. Before declaring equivalence at E1 or E2, the claim MUST state why no branching outside `S` occurs (at E1 this is discharged by the shared binary; at E2 it requires an explicit statement such as a content fingerprint taken before the uncovered path). A boundary declaration of the form "this fingerprint does not cover write-back / the concurrent path" is a REQUIRED part of the declaration, not a footnote.

**Invalid declarations (counter-example list).** §9.7 states what a declaration must contain; these forms are declared INVALID regardless of their completeness:

1. **Counting identity is not set identity** — equal cardinality across two sides does not establish equal membership (CoreSwap #162: `emptySec` is a count, not the set).
2. **Same caliber is not absence of spurious difference** — two numbers taken under nominally the same carrier may still differ by an unregistered factor.
3. **A plausible comparability narrative is not verification** — an explanation that "this explains the number" MUST itself be checked by an independent cheap sample before it is used as a premise (CoreSwap #162: a reasonable-sounding "46/46 = ABI switch" was refuted by a 10-line log sample).
4. **Presence of the required premise is not its satisfaction** — a grep proving a switch *exists* does not prove it was *in effect* during the measurement (CoreSwap #162).

Rationale (CoreSwap #162 + #115 + MT8/9/10 + #107 + #113): §9.7 as written is self-declared — a wrong declaration had no mechanical interception point, and interleaving form had no place in the three elements at all. The tier + key-set form gives the declaration something that can be refuted; the honesty clause imports the fact that the underlying relation is partial.

### 9.8 Verification Temporality (v0.22)

A verification action is not a side-effect-free observation. Every action that mutates disk state MUST register its **effect** and its **inverse**:

1. **`in-place` effects** — overwriting an existing log or artifact, rebuilding an index in place, changing a gate default, mutating a shared execution environment. Each MUST carry either
   - an **inverse** that is addressable on disk (an archived copy path, a renamed label, an environment-restore record), or
   - an explicit **irreversible declaration with a reason** (in the shape of the §9.5 honesty statement).
   Writing an inverse MUST NOT be fabricated to satisfy the clause: an inverse that does not actually restore the prior state is worse than a declared irreversible effect, because it makes a false claim (§7 behavior code).
2. **`derived` effects** — a newly created artifact under a fresh name, a new temporary directory, a new log path. These are compliant by construction (the inverse is identity: discard).

**What this clause does NOT require.** Automatic rollback, snapshotting, or any runtime mechanism that reverts verification side effects. Execution is host scope ([§16.2](#162-boundary)); the protocol requires the effect and its inverse to be **registered and addressable** — the same shape as the [§5.5](#55-source-provenance-v03) Source Artifact Requirement, one level up: §5.5 requires the *evidence for a claim* to be addressable; §9.8 requires the *inverse of a side effect* to be addressable. Automatic reversion is forbidden precisely because a failed round's artifacts are the more valuable evidence (a reverted failure is an unrecoverable failure).

**Mechanical form.** A verification action is compliant iff, for every `in-place` effect it declares, a `source`-addressable inverse (or an explicit irreversible declaration) exists. The inverse pointer is carried as its **own field on the verification record** — it is NOT appended to the `source=` string: `source` answers "which record backs this claim", the inverse answers "how was this side effect contained", and merging two different facts into one string muddies both the semantics and the scan gate.

Rationale (CoreSwap #144/#146 + three recorded evidence losses): the protocol constrained conclusions (§15.4 supersession), capability (§9 degraded modes) and round counts (§9.4), but said nothing about what a verification action leaves behind. The failures were real and recorded: a first-round log overwritten by a same-label rerun, a half-written log, a lost failed first run — and because no registry existed, the judge's three-source baseline could not check for the absence of an inverse it had never been told about. The source paper's own boundary is the accurate claim: the inverse's correctness is an obligation on the author, not a property the runtime verifies — so what this protocol adds is a **declaration** guarantee, honestly labeled as such.
...
- Current version: **v0.22** — pre-stable. All components subject to change based on practice feedback.
...
|-------|----------|-------------|--------|
| "Anchors MUST be removable by deleting a single dependency" | §2.4 | Python stub tested; TS/C++ comment-form inert by construction; Go/Java share the C++ line-comment form (v0.16 extractor wired); Rust not supported by design (native verification, v0.16) | ⚠️ scoped (Python verified; comment-form languages by construction) |
...
| "A Host MUST NOT restructure its framework to integrate" | §16.2 | Boundary clause; **DSH host adaptation (v0.18, 2026-08-13) integrated without restructuring** — the `dsh/` subtree adds an adaptation layer, the DSH framework itself is untouched | ⚠️ scoped (DSH implemented) |
| "A Host MUST NOT fork Anchorlaw; use §12 challenge or new interface point" | §16.2 | Boundary clause; **DSH adaptation filed its project-level-plugin need through the host feedback path instead of forking** (deepseek-ai/deepseek-harness discussion #306, 2026-08-13) | ⚠️ scoped (DSH feedback path used) |
...
| "Host MUST implement the four interface points" | §16.1/§16.3 | **DSH (dsh/ subtree, v0.18, 2026-08-13) implements all four**: execution point (4 anchorlaw_* tools → anchorlaw CLI), artifact paths (`.artifacts/`/`.investigations/` conventions), confirm hook (anchorlaw preset Judge persona — `confirmed` only by the human), trigger point (11 anchor-* skills + subagent delegation); guarded by selfcheck (6 items, incl. fail-closed preset-row resolvability — 2026-09-09 upstream rename `dsh-workflow-worker-thread` → `dsh-workflow-ptc`). Pipeline practice data pending | ⚠️ scoped (DSH implements; practice pending) |
...
| "Retry cap counts hypothesis rounds, not engineering fixes" | §9.4 (v0.7; v0.13 amended) | CoreSwap SearchTree port (3 crash iterations, all engineering) adjudicated by host human; v0.13 amended — the cap counts hypothesis rounds **without new data-layer evidence** (evidence saturation); engineering fixes never consume it | ⚠️ scoped (first-host practice) |
...
| "The Judge derives acceptance criteria from the input contract (confirmed requirements + technical-constraint specification) before implementation begins" | §15.4 (v0.10; v0.11 amended; v0.14 amended) | Normative clause (acceptance criteria first); v0.11 amended — criteria derive from the external input, not from in-protocol requirements discovery; v0.14 amended — input contract carries customer-confirmable facts only, architecture design is stage-1 output | ⚠️ scoped (unverified) |
| "The pipeline terminates on the Judge's nod (criteria satisfied), not on round counting; a criterion unmet across iterations MUST escalate to the human (§12 challenge or amendment)" | §15.4 (v0.10) | Normative clause (Judge-nod termination, C); **superseded in v0.12** — the "not on round counting" form relied on the Judge recognizing persistent failure; v0.12 restores a mechanical 3-iteration fallback on the same criterion | ⚠️ superseded (v0.12) |
| "Pipeline iteration on the same acceptance criterion is capped at 3; iteration 4 MUST halt the pipeline, and the Judge MUST submit a detailed report of the review situation and unresolved issues for the human to decide" | §15.4 (v0.12; v0.15 amended) | Normative clause (termination gates, C mechanical fallback); v0.15 amended — the escalation halts the whole pipeline and hands the decision fully to the human (no Judge pre-classification) | ⚠️ scoped (unverified) |
...
| "Anchorlaw consumes a confirmed requirements document + technical-constraint specification as its input contract; requirements discovery is external (a separate requirements protocol); architecture design is produced at pipeline stage 1" | §15.1 (v0.11; v0.14 amended) | Normative clause (input contract, stage 0, v0.14 layering); no host has exercised the handover yet | ⚠️ scoped (unverified) |
| "Derived acceptance criteria MUST be persisted as an artifact under the artifact store (default `.artifacts/`) for delivery-acceptance traceability" | §15.1 (v0.11) | Normative clause (input contract stage 0); consistent with §5.5 source artifact | ⚠️ scoped (unverified) |
| "Within the input-contract domain, programming is constructive — it converges on pre-defined acceptance criteria; the claim does not assert universality (reverse engineering is exploratory and outside the domain)" | §15.1 (v0.13) | Registered after §12 challenge (RE report: unregistered universal claim; CoreSwap SearchTree/biome tie-break as mixed-task counter-evidence); RE explicitly outside the domain | ⚠️ scoped (input-contract domain) |
| "The input contract carries customer-confirmable facts (claims, boundaries, terminology, technical constraints); architecture design (modularization, dependency direction, interfaces) is produced at pipeline stage 1" | §15.1 (v0.14) | Normative clause (stage-0/stage-1 boundary, v0.14 layering); no host has exercised it | ⚠️ scoped (unverified) |
| "An input contract counts as *confirmed* only when semantically converged — every claim translatable to a verification assertion, no open hypotheses the implementation depends on" | §16.1 (v0.13; v0.14 generalized) | Normative clause; generalized from the v0.13 RE handover criterion to be protocol-neutral (names no upstream framework) | ⚠️ scoped (unverified) |
| "Go and Java anchors use line-comment declaration (`// @anchor.*`) with an independent probe/test binary as validation carrier" | §2.4/§13 (v0.16; v0.17 scoped) | Reference extractor wired (same regex as C++); no real-project practice yet; **v0.17: annotation-extraction ONLY — P1-P6 not mapped, no defect detection** | ⚠️ scoped (annotation-extraction only; defect detection pending) |
| "P7-P10 reliability-risk patterns (LIFECYCLE/STATE_MACHINE/PATH-COORDINATION/COMPLEXITY) are defined language-agnostically; implementations map them per language and Level 1 does not require them" | §6 (v0.17) | Definitions driven by the Reasonix/Go practice audit (real defect classes outside P1-P6); no implementation yet | ⚠️ scoped (defined; implementation pending) |
| "An unparseable source file is a tool-level parse-error marker (INFO), never a P1-P6 defensive pattern" | §6 (v0.17) | §12 challenge (Reasonix/Go audit: SyntaxError misclassified as swallowed-exception); reference scanner implements PARSE_ERROR | ⚠️ scoped (reference implementation) |
| "Rust is not supported by design — its compiler, borrow checker, and test framework already provide the verification this protocol adds elsewhere" | §2.4 (v0.16) | Design decision; an anchor layer would duplicate native guarantees | ⚠️ scoped (design decision) |
| "Any candidate+ conclusion overturned by later evidence MUST be expressed as a supersession record; the chain MUST mechanically answer the currently valid conclusion and its history" | §15.4 (v0.20) | Normative clause (supersession chain semantics); acceptance = one overturn = 1 supersession record + 0 original-text rewrites; carrier is host scope | ⚠️ scoped (normative clause, host practice pending) |
| "Any quantitative alignment/consistency metric MUST declare the comparison basis (carrier / coverage / comparability with prior metrics)" | §9.7 (v0.20) | Normative clause (comparability statement); acceptance = spot-checked quantitative conclusions carry all three elements with historical comparability explicit | ⚠️ scoped (normative clause, host practice pending) |
| "Handover documents MUST distinguish verified conclusions from unverified hypotheses (labeled); the inheritor MUST run one cheap independent verification before using a direction-level conclusion as a premise" | §16.3 (v0.20) | Normative clause (host handover validation, checklist item 7); targets the M11/M14 root cause (hypothesis crossing a session boundary as axiom) | ⚠️ scoped (normative clause, host practice pending) |
| "Every in-place side effect of a verification action MUST carry an addressable inverse or an explicit irreversible declaration; automatic rollback is forbidden" | §9.8 (v0.22) | Normative clause (verification temporality); trigger evidence is first-hand (three recorded CoreSwap evidence losses; `merge_index.py` rebuild-style write is a live data-loss risk); acceptance = an in-place effect whose inverse is addressable on disk, and an irreversible effect with a stated reason. **Not yet implemented by any host; the inverse pointer's carrier is deliberately a record field, not the `source=` string.** | ⚠️ scoped (normative clause, no compliant implementation yet) |
| "An acceptance criterion carries the external facts it depends on; when a precondition lapses the criterion is suspended and its dependents are marked premise-expired, and no status is auto-changed" | §15.1 / §15.4 (v0.22) | Normative clause (criterion preconditions); trigger evidence = CoreSwap #161 (judge three-source also missed the lapse), #156, M11; the status-immunity half is bounded by the existing one-way transition rule and the confirm hook, both of which are separately verified | ⚠️ scoped (normative clause, host practice pending) |
| "Two executions may be judged observationally equivalent only within a declared equivalence tier, comparing only the mutually declared observation-key set; equivalence of the underlying relation is partial (reflexivity not preserved)" | §9.7.1 (v0.22) | Normative clause (equivalence tiers + partial-equivalence honesty clause + invalid-declaration list imported from #162); practice precedents #115/#150/#116/#111/#148 already exhibit the tier shapes | ⚠️ scoped (normative clause, tier adoption pending) |
| "A halted pipeline is terminal: its state MUST NOT be inherited as the basis of a subsequent round, and the halt MUST terminate with a mechanical signal" | §15.4 PI-1 (v0.22) | Normative clause (process invariant); a script-level precedent exists at the practice host (#160) and the clause is deliberately host-neutral | ⚠️ scoped (normative clause, no conformance test yet) |
| "Multiple parallel candidates for one criterion MUST converge on a unique adjudication endpoint before any is cited as a conclusion" | §15.4 PI-2 (v0.22) | **Registered as unverified by construction**: its premise (acyclic criterion-dependency graph) is an assumption in the source analysis, not a derived property; the clause itself mandates a ≤1-round enumeration before adoption and an amendment path if cycles are found | ⚠️ unverified (premise unregistered — stated, not hidden) |
| "Live references to this protocol MUST be checkable at the decidable layer (version currency, clause-number existence, anchor resolvability); semantic compatibility is out of scope as undecidable, and historical archives are exempt" | §14.7 (v0.22) | Normative clause (reference integrity, applied to the protocol's own citations); scope deliberately limited to the decidable layer; checker not implemented (owner: protocol maintenance) | ⚠️ scoped (normative clause, checker not implemented) |
...
| Property | Python | TypeScript | C++ | Go / Java (v0.16) |
|----------|--------|-----------|-----|------------------|
| **Declaration site** — where the anchor attaches to code | decorator | JSDoc comment | line comment (`// @anchor.*`) | line comment (same as C++) |
| **Validation carrier** — what executes the check | `test_fn` lambda | runtime assert | independent probe binary | independent probe/test binary |
| **Removal** — behavior without the protocol | no-op decorator | inert comment | inert comment | inert comment |

**P1-P6 mapping (v0.17 claim correction)**: Python/TS implement full
P1-P6 scanning; comment-form languages (C++/Go/Java) currently provide
annotation-extraction ONLY — P1-P6 is not mapped to them (no defect
detection). P7-P10 (v0.17 reliability-risk patterns) are language-agnostic
definitions; implementations map them per language.
...
> **Scope (v0.19):** §14 is the Anchor Skill Manifest — HOW agent skills are
> organized — NOT the knowledge sink. Anchorlaw does not accumulate project
> knowledge (that is a host / separate knowledge mechanism's responsibility,
> e.g. a framework's §6 knowledge base). Knowledge-content recording is out of
> scope here; the only knowledge-adjacent field in Anchorlaw is the noise card's
> `discovery`/`curriculum` ([§3](#3-noise-card-json-schema)), and those are
> verification backtracking, not a general knowledge store.
>
...
### 14.7 Reference Integrity (v0.22)

The protocol applies its own interface-drift analysis to its own citations.
A *live reference* — a document, skill body, or host manifest that cites this
protocol or its clauses — MUST be checkable at the **decidable layer only**:

1. **Version** — the cited protocol version is the current baseline (a stale
   version citation is interface drift: the reference is satisfied at the name
   level while its content no longer matches).
2. **Clause existence** — every cited clause number still exists, and still
   means what the citing text implies. A number that now denotes different
   content is the clause-level form of key collision, and is the failure mode
   that repeated amendment produces (a clause amended across several versions
   while the citing text stayed put).
3. **Anchor resolvability** — a cited section anchor resolves.

**Explicitly out of scope.** Semantic compatibility checking ("does this
citing text remain *correct* against the amended clause?") is undecidable and
MUST NOT be attempted mechanically; the paper's own analysis of structural
compatibility reaches the same conclusion for behavior contracts. **Historical
archives are excluded** — archived versions of the protocol and of host
documents are read as history and MUST NOT be rewritten to satisfy the check;
only live references are in scope.

**Ownership.** This check is protocol-maintenance scope (`anchor.maintain`),
NOT a universal host obligation: a host that cites the protocol is expected to
keep its live citations current, but implementing the checker is not a
condition of host compatibility. Rationale: three recorded drift forms
(CoreSwap #156 citation-lost-its-premise, #13 transcription distortion, #90
handover-item drift) plus repeated §15.4 amendment history; the check's
carrier already exists — the §11 audit table registers the verification status
of every universal claim, which is the natural home for "which clause is cited
where, and is it still the same clause".
...
**Judge-driven pipeline (v0.10; v0.11 amended; v0.13 scoped; v0.14 layered)** — within this
protocol's domain — the input contract is already confirmed — programming is
constructive: it converges on pre-defined acceptance criteria. The
constructiveness claim is **scoped to this domain**; it does not assert
universality ([§11](#11-universal-claim-audit-v04) audit entry, registered
after a §12 challenge). In contrast, reverse engineering is exploratory
(survey → discovery → convergence, unbounded verification is its nature) and
lies **outside** this protocol's domain; the two modes are complementary, not
exclusive — exploratory tasks must not be forced into the Judge-driven
pipeline ([§16.1](#161-definition) input-contract confirmation criterion). The workflow is
therefore **Judge-driven**. Requirements discovery is NOT part of this
protocol: it belongs to a separate requirements protocol (Scout-driven,
human-dialogue based), whose output — a confirmed requirements document plus
technical-constraint specification — is this pipeline's **input contract**.
The input contract carries what the customer can confirm as facts (claims,
boundaries, terminology, technical constraints: platform/language, external
dependencies, performance targets, customer-mandated architecture decisions);
it does NOT carry the architecture design (modularization, dependency
direction, interfaces) — that is produced inside this pipeline at stage 1
(Implementation spec) under the Judge's review (v0.14 layering). The
stage-0/stage-1 boundary: whatever the customer mandates and confirms is a
constraint (stage 0); whatever the pipeline decides freely is design (stage
1). Anchorlaw
consumes the external input and never re-discovers requirements. The pipeline
has four stages, each terminating on a "Judge nod" in the normal path;
same-criterion iteration is mechanically capped at 3 by the [§15.4](#154-consistency-contract) C gate:

0. **Input contract** — the confirmed requirements document +
   technical-constraint specification arrive from the external requirements
   protocol (host handover, [§16.1](#161-definition)); the Judge derives the
   acceptance criteria from this input before implementation begins ([§15.4](#154-consistency-contract)
   Acceptance criteria first). No implementation work may start without this
   input. The derived acceptance criteria MUST be persisted as an artifact
   under the artifact store ([§15.2](#152-isolation-semantics), default
   `.artifacts/`, e.g. `acceptance-criteria-<NNN>.md`) so delivery acceptance
   can be checked against them (traceability, consistent with §5.5 source
   artifact).
1. **Implementation spec** — the Judge dispatches Scouts (or Workers) to
   ...
   dependency direction, framework boundaries, interfaces. The Judge reviews the spec; only an approved spec
   enters planning.
...
- A subprocess MUST persist its outputs as artifacts before returning
  (artifacts are the verification-reproducibility carrier — they let a claim
  be re-checked against its data; they are NOT a general-knowledge store,
  which is a host / separate knowledge mechanism's responsibility).
...
- (v0.10; v0.11 amended) MUST fire at every pipeline stage boundary — the
  ...
  1. **Input contract acceptance** — Judge accepts the external requirements
     document + technical-constraint specification and derives the acceptance criteria
...
+ technical-constraint specification, [§15.1](#151-definition) stage 0) before
...
**Criterion preconditions (v0.22)** — every acceptance criterion carries, in
the criteria artifact ([§15.1](#151-definition) stage 0), the external facts it
depends on. Each precondition states three things:

- `key` — the identity of the observed object (which world / which artifact /
  which gate state / which provenance source),
- `expected` — the value or shape required for the criterion to be meaningful,
- `check` — a mechanically executable confirmation of that fact.

**Satisfaction and lapse.** A criterion is *active* only while its precondition
set holds. When a precondition lapses, the criterion MUST be **suspended** and
every conclusion citing it MUST be marked *premise-expired, pending
re-check* — it MUST NOT continue to be cited as a standing conclusion.

**Status is never auto-changed (hard boundary).** Suspension changes the
criterion's *citable* state, NOT any artifact's confidence status. The one-way
transition rule (`draft → candidate → confirmed`, [§15.4](#154-consistency-contract))
and the confirm hook ([§16.1](#161-definition)) remain absolute: a criterion
precondition lapse MUST NOT downgrade a status, MUST NOT revoke `confirmed`,
and MUST NOT be presented as a human decision. The human decides; the protocol
only stops the expired premise from being silently inherited.

**Trigger classification.** The transition of a precondition set's
satisfaction is classified by *what changed*, not by whether a value moved:
*activating* (unsatisfied → satisfied: the criterion becomes active/executable),
*deactivating* (satisfied → unsatisfied: the criterion is suspended),
*neutral* (otherwise). A value that changes while satisfaction is unchanged is
neutral and MUST NOT be treated as a premise event.

**Scope discipline (anti-fake-gate).** A precondition set MUST be drawn from
facts that (a) have already caused a lapse in practice, or (b) are already
checked mechanically by the collection pipeline — it is NOT required to be
complete. An initial empty set is compliant; what is forbidden is a criterion
that cites external state while declaring no precondition at all.

Rationale (CoreSwap #161 / #156 / M11): the three recorded failures share one
shape — the *observed object* of a criterion lapsed (world identity, a master
switch, a seed) while the mechanical read kept executing and produced a
normal-looking number. #161 is the decisive case: the judge's three-source
cross-check did not catch it either, so post-hoc review is demonstrably
insufficient. The §15.4 C gate counts iterations and cannot see a premise that
quietly expired. Preconditions convert a class of "discovered afterwards" into
"intercepted beforehand", and they attach to a mechanism the pipeline already
runs: the collection script's self-certification gate is already a
`σ ⊨ d` check that voids the round on any missing item — this clause lifts that
check from script-inline into the criteria artifact so a void can point back at
the criterion it invalidated.
...
- **B. Review opinion severity tiers** — review/judge opinions fall into
  ...
  3. **info** — notes and observations; recorded, may be ignored. An opinion
     ...
     round.
- **C. Judge-nod termination with mechanical fallback (v0.10; v0.12; v0.15
  amended)** — the pipeline terminates per stage on the Judge's nod:
  criteria satisfied (external test set green, no unresolved blocking
  opinion, dispositions recorded — A and B above), not on round counting.
  Return-and-fix is normal pipeline iteration: every return carries a review
  opinion the Worker acts on, so each iteration converges on the same
  pre-defined criteria (criteria-first, above).
  **Mechanical fallback (v0.12; the v0.9 round cap, restored and adapted; v0.15 amended)** —
  the iteration count on the SAME acceptance criterion MUST be tracked in
  the workflow plan (task list). After 3 iterations without the criterion
  being met — the review is still reporting unresolved issues — the pipeline
  MUST **halt entirely**: no further iteration is allowed, and the Judge MUST
  NOT continue fixing, re-classifying, or re-reviewing on its own. The Judge
  MUST submit a detailed report of the review situation and the unresolved
  issues to the human, and the **human decides** — the criterion itself may
  be wrong (challenge via
  [§12](#12-rule-challenge-process-v04) or human amendment), the approach may
  be wrong (return to planning), or the human redirects otherwise. The halt
  is unconditional, regardless of whether the Judge perceives progress.
  Rationale: the v0.10 Judge-nod-only form relied on the Judge recognizing
  "persistent failure" — the exact failure mode (repeated rounds without
  realizing) the gates exist to prevent. Criteria-first removes the cause of
  *criteria appearing after the fact*, but not the case of a
  *wrongly-defined criterion* that can never be met; the mechanical count is
  the fallback for that case. v0.15: the escalation hands the decision fully
  to the human — the Judge's own classification at round 4 ("criterion wrong
  vs. approach wrong") is itself a convergence judgment that can keep
  failing; the halt + detailed report removes that residual loop. The
  per-function retry
  cap of [§9.4](#94-retry-cap-anti-entropy) (v0.13: evidence saturation — 3
  consecutive hypothesis rounds without new data-layer evidence) applies in
...
**Process invariants (v0.22)**

- **PI-1 — a halt is terminal.** A halted pipeline (gate C) MUST NOT be
  continued as though its state were valid: its state MUST NOT be inherited as
  the basis of a subsequent round, and the halt MUST terminate with a
  mechanical signal (non-zero exit or an equivalent host-level marker) rather
  than a message alone. The halt MUST resolve to exactly one of the three
  destinations gate C already names (human adjudication / §12 challenge /
  back to planning); leaving it undecided is what allows silent inheritance.
  Rationale: CoreSwap #160 — a gate that only printed let a VOID round's
  contaminated artifacts become the next round's basis, so the halt never
  became a terminated state. The clause is host-neutral on purpose: the
  protocol names the *invalidity of the halted state*, not any host's
  continuation mechanism.
- **PI-2 — unique adjudication endpoint (registered, unverified).** Multiple
  parallel candidates for one criterion (a fan-out's `.bN` set) MUST converge
  on a single adjudication endpoint, and no candidate may be cited as a
  conclusion before that convergence. **Verification status: unverified.** The
  invariant's premise is that the criterion-dependency graph is acyclic; the
  source analysis states acyclicity as an *assumption*, not a deliverable of
  the definition — self-referential criteria violate it. Hosts adopting PI-2
  MUST first perform a cheap enumeration of the active criteria dependency
  graph (≤1 round) and register the result; if cycles exist, the clause MUST be
  amended to "cycles permitted but registered" rather than enforced as a ban.
  This precondition is stated rather than hidden because the protocol requires
  registered premises ([§11](#11-universal-claim-audit-v04)), not assumed ones.
...
**Conclusion supersession chain (v0.20)** — a `candidate`-or-higher conclusion
overturned or refined by later evidence MUST be expressed as a **supersession
record**, not by editing or deleting the original text:

- The superseded conclusion is annotated `superseded_by: <new-conclusion-id>`;
  the new conclusion is annotated `supersedes: <old-conclusion-id>` plus a
  one-line overturning reason. The original body is NEVER deleted or rewritten
  (evidence-chain integrity; same auditability spirit as the First Law).
- The chain MUST mechanically answer: "what is the currently valid conclusion
  about X, and what is its history?" — walking `supersedes` links to the chain
  head yields the valid conclusion; the full walk yields the evolution history.
- Relation to [§5.3](#53-staleness-detection): staleness detection (time /
  source decay) is one TRIGGER of supersession; the supersession record is its
  landing action. Relation to the §11 audit: a claim audit walks the chain to
  the head (currently valid conclusion), not an isolated claim.
- Carrier is a host implementation detail ([§15.5](#155-interface-surface-audit-v06)):
  the protocol constrains the supersession SEMANTICS (bidirectional links +
  reason + immutable original), not where records live or who performs them.
- Migration: existing conclusions are lazily linked (new conclusions MUST;
  old ones gain links when next touched).

Rationale (CoreSwap M14/M16): a handover's "mechanism direction (candidate,
unverified)" was treated as an axiom by the next session and drove a wasted
round; the overturning of that direction had no supersession expression — only
a hand-written "closed/voided" marker — so nothing could mechanically answer
which conclusion was currently valid.
...
requirements + technical-constraint specification as its input contract
...
**Input-contract confirmation criterion (v0.13; v0.14 generalized)** — an
input contract counts as *confirmed* only when it is **semantically
converged**: every claim is translatable to a verification assertion, and the
requirements carry no open hypotheses that the implementation depends on
(unresolved unknowns are declared as explicit boundaries, not left to emerge
during implementation). An upstream host that would hand over an unconverged
contract (e.g. acceptance criteria still emerging from discovery) MUST NOT
enter the Judge-driven pipeline until the exploratory portion has converged:
it either (a) completes the discovery first and hands over a confirmed
contract, or (b) calls Anchorlaw only for the determined sub-portions whose
semantics are already stable. Handing an unconverged contract into the
pipeline forces it to behave like an exploration loop — out of domain
([§15.1](#151-definition)) — and collides with the acceptance-criteria-first
rule ([§15.4](#154-consistency-contract)). The criterion is protocol-neutral:
it names no upstream framework — any upstream (a human requirements protocol,
a discovery-driven framework, or the host's own process) must meet it before
handover.
...
7. **Host handover validation (v0.20)** — handover documents distinguish
   verified conclusions (judge-reviewed) from mechanism-directions/unverified
   hypotheses, and the latter are labeled with their verification status; the
   inheriting session MUST run one cheap independent verification (≤1 round of
   cost) on any direction-level conclusion it intends to inherit BEFORE using
   it as a reasoning premise.

Rationale (CoreSwap M11/M14): the common root cause of both a wasted round and
a thrice-repeated mistake was an unverified hypothesis crossing a session
boundary and being promoted to an axiom. Handover is part of the host contract
([§16.1](#161-definition)); the existing checklist only covered "what was
handed over", not "which parts of it are axioms vs assumptions".
