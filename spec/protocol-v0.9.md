...
- Current version: **v0.9** — pre-stable. All components subject to change based on practice feedback.
...
| "Convergent tasks (writing, verification) default to `inline`; the main agent writes with full context" | §15.3 (v0.8) | Execution-mode selection principle; `anchor.write`/`anchor.test` inline in the manifest; conformance tests assert body Execution lines match | ⚠️ scoped (reference implementation) |
| "The only sanctioned reference subagent role is the review gate (judge)" | §15 (v0.8) | `anchor.scout`/`anchor.worker` removed from the reference implementation; `anchor.judge` remains kind:role | ⚠️ scoped (reference implementation) |
...
| "Verification rounds on one issue are capped at 3, counted in the plan; round 4 MUST escalate to the human" | §15.4 (v0.9) | Normative clause (termination gates, C); extends §9.4 retry cap from per-function engineering to the verification loop | ⚠️ scoped (unverified) |
...
| **Subprocess (executor)** | Executes one protocol action in isolation | Its full working context MUST NOT enter the main session; only the final answer and artifact references return |
...
**Execution-mode selection principle (v0.8):** convergent tasks — writing,
verification, single-threaded progress — default to `inline`: the main agent
holds full context and executes them itself. Divergent tasks — survey,
multi-hypothesis analysis, large-scale scanning — MAY use `subprocess`.
`subprocess` is an optimization for divergent work, not a programming default.
...
**Judge trigger points (v0.9)** — the review gate is not only a closing gate;
it is a mandatory checkpoint at specific decision points. A judge review:
...
**Verification termination gates (v0.9)** — the convergence gate
(verification → review → commit) MUST terminate on mechanical criteria, not on
the exhaustion of review opinions. This section exists because a review-driven
...
- **B. Review opinion severity tiers** — review/judge opinions fall into
  ...
  3. **info** — notes and observations; recorded, may be ignored. An opinion
     ...
     round.
- **C. Verification round cap** — verification rounds on the same issue MUST be
  counted explicitly in the workflow plan (task list). After 3 rounds without
  convergence on the same issue, the agent MUST stop and escalate to the human;
  further rounds are prohibited. This extends the retry cap of
  [§9.4](#94-retry-cap-anti-entropy) from per-function engineering to the
