...
| 2026-09-18 | host `@deepseek-ai/dsh` 的 `lib/profile-boot-*.js`（`homePatchPath()`）+ `@deepseek-ai/dsh-app-boot` README | **修正 `scripts/install.ps1` 的两处事实性注释**（纯注释，无行为变更）：原文称 `<dshHome>/profiles/<profile>/cordis.patch.yml` 是「DSH 唯一读取的用户补丁层；home 级 `~/.dsh/cordis.patch.yml` 宿主不读」。实际宿主**读两个用户补丁层**——home 级文件在 per-profile 层**之后**应用、优先级更高（README 原文 "applied after every bundle layer (per-profile first, then the home-level file, which therefore outranks it)"；`homePatchPath()` 在 0.1.5-rc.2 与 0.1.6-alpha.2 均存在，**不是新版本变化**）。改后注释：本安装器只写 per-profile 层（home 级与其它框架、本机偏好共享，不是它该改的），并如实登记一个既有注意点——第 4 步合并经 `yaml.safe_dump` 往返，**不保留补丁文件里的注释**（行为未改）。 | —（非技能正文） |
