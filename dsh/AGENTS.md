...
3. 若改动涉及协议语义：协议正文在上层 `../spec/protocol-v0.21.md`（§8 Maturity / §11 全称声称审计 / §14 Skill Manifest），证据必须跟着走；技能正文直接改本目录 `skills/`（DSH 技能唯一事实源，协议 §14 是宿主无关的技能规范）。
...
- **仓库根（`../`）**：协议正文（`spec/protocol-v0.21.md`）、Python/TS 实现、Reasonix 版存档（`archive/reasonix/`）。
