...
> Reasonix 在本仓库工作时自动加载本文件。本文件是**索引**——协议知识按需加载（协议 v0.7 §14 Anchor Skill Manifest + §15 Execution Topology），铁律正文在对应 skill 里，不在本文件常驻。
...
3. 若改动涉及协议文档：同步核对 [§8 Maturity](spec/protocol-v0.7.md#8-maturity) 与 [§11 全称声称审计表](spec/protocol-v0.7.md#11-universal-claim-audit-v04)，证据必须跟着走
...
## 二、Skill 触发索引（协议 v0.7 §14-§16，参考实现 `.reasonix/skills/`）
...
| 路径 | 内容 |
|------|------|
| `spec/protocol-v0.7.md` | **协议当前版**（v0.1/v0.3/v0.4/v0.5/v0.6 留档历史，不改） |
...
## 四、通用协议性铁律（最高优先级，细节见 `anchor.maintain`）
